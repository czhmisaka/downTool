...
class down():
    ...
    def activeThread(self):
        ...
        for x in self.status:
            ...
            if x['now'] != '等待任务':
                num = num + 1
        return [
            str(round(num / len(self.status) * 100, 3)) + ' %',
            str(round(num1 / len(self.status) * 100, 3)) + ' %'
        ]
    ...
    def workProcess_create(self, threadStatus):
        ...
        if self.key_Keep:
            ...
            self.threadList[
                threadStatus['tag']]['thread'] = _downTool_commonThread(
                    self.workProcess, (tag, 'name'),
                    'name' + str(threadStatus['tag']))
    ...
    def workProcess(self, tag, name):
        ...
        while (self.key_Keep):
            ...
            if self.__downLoadStart(deal['url'], deal['path'], tag,
                                    deal['isLarge'], deal):
                self.__changeStatusByTag(tag, '完成下载', deal['path'])
                continue
            else:
                if deal['isLarge'] == False:
                    self.addMission(deal['url'],
                                    path=deal['path'],
                                    reDown=deal['reDown'] + 1,
                                    isLarge=deal['isLarge'])
    ...
    def saveHistory(self):
        ...
        '''
        try:
            data = {'key': self.taskKey, 'list': self.taskList}
        ...
        try:
            if isLarge:
                ...
                if reDown < self.reDownMax:
                    ...
                    self.logTag("error : 任务重复添加 reDown:" + str(reDown) +
                                ' url: ' + url + ' path: ' + path)
                    return False
        except:
            self.logTag("error : 任务添加失败 reDown:" + str(reDown) + ' url: ' +
                        url + ' path: ' + path)
            return False
        else:
            self.logTag("success : 任务添加成功 reDown:" + str(reDown) + ' url: ' +
                        url + ' path: ' + path)
            return True
            ...
            for chunk in r.iter_content(chunk_size=self.chunk_size):
                if chunk:
                    ...
                    if time.time() - time1 > 0.25:
                        p = count / length * 100
                        speed = self.__formatFloat(
                            (count - count_tmp) / 1024 / 1024 / 0.25)
                        count_tmp = count
                        self.__changeStatusByTag(
                            tag, '正在下载', path,
                            str(speed) + 'MB/s',
                            str(int(count / length * 100)) + '%')
                        time1 = time.time()
    ...
    def downLoad_LSize(self, url, path, tag, start, end):
        ...
        try:
            ...
            self.__changeStatusByTag(tag, '超时', path)
            self.logTag("Error<<downLoad_LSize()>> -path:" + path + "-url:" +
                        url)
            return False
        except:
            self.__changeStatusByTag(tag, '其他错误', path)
